...
from megatron.utils import update_rotary_pos_emb
import deepspeed
...
def convert_hf_to_mega_ds():
    ...
    tokens = torch.load("script/tokens.pt").cuda()
    labels = torch.load("script/labels.pt").cuda()
    position_ids = torch.load("script/position_ids.pt").cuda()
    mask = torch.ones_like(labels).cuda()
    mask[labels == 0] = 0
    # attention_mask = torch.load("script/attention_mask.pt").cuda()
    attention_mask = torch.ones_like(labels).cuda()
    def fake_iter(*args):
        while True:
            yield (tokens, position_ids, attention_mask), (labels,mask)
    def fake_dt():
        while True:
            yield tokens,
    dt_iter = fake_iter()
    def fn_iter():
        ...
        return batch_fn
    dt = fake_dt()
    ds_engine.module._megatron_batch_fn = fn_iter()
    ds_engine.set_batch_fn(ds_engine.module._megatron_batch_fn)
    o = ds_engine.eval_batch(dt)
    if torch.distributed.get_rank() == 0:
        from IPython import embed;embed()
    torch.distributed.barrier()

    output_tensor, _ = ds_engine(tokens, position_ids, attention_mask, labels=labels)
