...
def get_batch(data_iterator):
    ...
    # Unpack.
    labels = data_b['labels'].long()[:,1:].contiguous().cuda()
    tokens = data_b['input_ids'].long()[:,:-1].contiguous().cuda()
    tokens = tokens.masked_fill(tokens >= 32000, 0)
    labels = labels.masked_fill(labels >= 32000, 0)
    ...
    loss_mask = loss_mask & labels.ne(0)
