...
finetune_args="torchrun --nproc-per-node=${gpus} --nnodes ${nnodes} --master-addr=${master_addr} --master-port=12306 --rdzv-id=1 --rdzv-backend=c10d --rdzv-endpoint=${master_addr}:12221 ./${filename} \
--finetune"

finetune_args="torchrun --nproc_per_node=${gpus} --nnodes ${nnodes} --master_addr=${master_addr} --master_port=11306 ./${filename} \
--finetune"
# --untie-embeddings-and-output-weights \
