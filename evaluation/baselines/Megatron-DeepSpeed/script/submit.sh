...
if [ -z "$AVAIL" ] || [ "$NODE_COUNT" -lt "$WORLD_SIZE" ]; then
  ...
  pdsh -R ssh -w "$NODES" "export LOG_FILE=$LOG_FILE;export MASTER_ADDR=$MASTER_ADDR;export WORLD_SIZE=$WORLD_SIZE; export CP_SIZE=$CP_SIZE;export MODEL=$MODEL; cd `pwd` && $1"
