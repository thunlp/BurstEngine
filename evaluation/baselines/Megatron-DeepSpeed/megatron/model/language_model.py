...
def parallel_lm_logits(input_, word_embeddings_weight, parallel_output,
    ...
    if args.tensor_model_parallel_size > 1:
        ...
        logits_parallel = torch.matmul(input_parallel, word_embeddings_weight.t())
        if bias is not None:
            logits_parallel += bias
...
class TransformerLanguageModel(MegatronModule):
    ...
    def load_state_dict(self, state_dict, strict=True):
        ...
        # Pooler.
        if 'language_model.output_layer.weight' in state_dict:
            state_dict['output_layer'] = {}
            state_dict['output_layer']['weight'] = state_dict.pop('language_model.output_layer.weight')
        if self.post_process:
            if self.add_pooler:
