...
def _burst_fixedlen_qkvsplited_attn(*args, **kwargs):
...
def _burst_fixedlen_kvpacked_attn(
    *args,
    **kwargs,
):
    return _flash_float32_compatibility_wrapper(
...
_attn_ops_bindings = {
    ...
    AttnType.Burst: {
        AttnOpType.FixedLenQKVSplited: _burst_fixedlen_qkvsplited_attn,
        AttnOpType.FixedLenKVPacked: _burst_fixedlen_kvpacked_attn,
    },
...
def _select_attn_op(op_type: AttnOpType) -> Tuple[AttnType, Callable]:
    ...
    enable_burst = gpc.config.parallel.burst.enable
    ...
    if gpc.config.model.get("use_flash_attn", False):
        if device_backend == AcceleratorType.GPU and gpu_flash_attn_impl:
            if enable_2D_sp is True:
                if enable_burst is False:
                    attn_type = AttnType.SlidingWindowZigZagFlash
                else:
                    attn_type = AttnType.Burst
            else:
