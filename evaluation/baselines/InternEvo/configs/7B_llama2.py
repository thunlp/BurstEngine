...
DO_ALERT = False
import os
from configs.model_size_utils import get_config

VOCAB_SIZE = 32000
SEQ_LEN = int(os.environ.get("SEQ_LEN", 32768))
size = os.environ.get("MODEL_SIZE", "7b")
conf = get_config(size)

if os.environ['RANK'] == "0":
    print("****************MODEL_CONFG**************")
    for key, value in conf.items():
        print(f"{key}: {value}")
    print("**************MODEL_CONFG_END****************")

VOCAB_SIZE = conf["VOCAB_SIZE"]
HIDDEN_SIZE = conf["HIDDEN_SIZE"]
NUM_ATTENTION_HEAD = conf["NUM_ATTENTION_HEAD"]
NUM_KV_ATTENTION_HEAD = conf["NUM_KV_ATTENTION_HEAD"]
MLP_RATIO = conf["MLP_RATIO"]
NUM_LAYER = conf["NUM_LAYER"]
...
CHECKPOINT_EVERY = 50
selective_checkpoint = os.environ.get("sele_ckpt", "0") == "1"
ckpt = dict(
...
lr_scheduler = dict(
    ...
    init_steps=0,  # kptimizer_warmup_step
    warmup_ratio=0.01,
    eta_min=1e-5,
    last_epoch=-1,
)
...
hp_size = int(os.environ.get("HP_SIZE", 1))
method = os.environ.get("METHOD", "loong")
print(f"[parallel] cp_size: {cp_size}, world_size: {world_size}, hp_size: {hp_size}, method: {method}")
window_size = cp_size if hp_size > 1 else cp_size // 4
cce_loss = False
parallel = dict(
    zero1=dict(size=-1),
    tensor=dict(size=hp_size*cp_size, mode="isp"),
    pipeline=dict(size=1, interleaved_overlap=True),
    weight=dict(size=hp_size*cp_size, overlap=True, memory_pool=False),
    burst=dict(
        enable="burst" in method,
    ),
    sequence_2D=dict(
        enable=True,
        head_size=hp_size,
        window_size=window_size,
        context_size=cp_size,
...
tensorboard_folder = f"./tensorboard/llama{size}_{world_size}xgpu/_cp{cp_size}_hp{hp_size}_{method}_seq{SEQ_LEN}"
