...
from megatron.core.transformer.utils import make_sharded_tensors_for_checkpoint, _SeqAllToAll
...
class TEDotProductAttention(te.pytorch.DotProductAttention):
    ...
    def forward(
        ...
        if self.te_forward_mask_type:
            ...
            causal = "causal" in attn_mask_type.name
            if use_burst:
                from burst_attn import burst_attn_func
                # only local mode
                assert self.qkv_format in ['sbhd', 'bshd']
                if self.qkv_format == 'sbhd':
                    query = query.transpose(0, 1)
                    key = key.transpose(0, 1)
                    value = value.transpose(0, 1)
                if use_ulysses:
                    ulysses_group = self.args.cp_groups_double[1]
                    burst_group = self.args.cp_groups_double[0]
                    query = _SeqAllToAll.apply(ulysses_group, query, 2, 1)
                    key = _SeqAllToAll.apply(ulysses_group, key, 2, 1)
                    value = _SeqAllToAll.apply(ulysses_group, value, 2, 1)
                    core_attn_out = burst_attn_func(
                        query,
                        key,
                        value,
                        None,
                    ...
                    core_attn_out = _SeqAllToAll.apply(ulysses_group, core_attn_out, 1, 2)
                else:
                    core_attn_out = burst_attn_func(
                        query,
                        key,
                ...
                if self.qkv_format == 'sbhd':
                    ...
                    key = key.transpose(0, 1)
                    value = value.transpose(0, 1)
                query = _SeqAllToAll.apply(self.args.cp_groups, query, 2, 1)
                key = _SeqAllToAll.apply(self.args.cp_groups, key, 2, 1)
                value = _SeqAllToAll.apply(self.args.cp_groups, value, 2, 1)
                core_attn_out = flash_attn_func(query, key, value, 0.0, None, causal, (-1, -1))
                core_attn_out = _SeqAllToAll.apply(self.args.cp_groups, core_attn_out, 1, 2).flatten(-2, -1)
