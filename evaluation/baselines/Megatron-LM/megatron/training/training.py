        ...
        * (
            ...
            (
                (
                    1
                    + (args.num_query_groups / args.num_attention_heads)
                    + (args.seq_length / args.hidden_size) * (0.5 if args.causal_tflops else 1)
...
def pretrain(
    ...
    args = get_args()
    timers = get_timers()
    if args.use_burst or args.use_ulysses:
        # cp_g = [torch.distributed.new_subgroups(group=mpu.get_context_parallel_group())[0] for i in range(2)]
        # args.cp_groups = cp_g if args.create_dq_comm else cp_g[0]
        args.cp_groups = mpu.get_context_parallel_group()

    if (args.double_ring and args.use_burst) or (args.use_ulysses and args.use_burst):
        local_size = int(os.environ.get('LOCAL_WORLD_SIZE'))
        cp_size = mpu.get_context_parallel_world_size()
        assert cp_size % local_size == 0 
        group_ranks = torch.tensor(list(range(torch.distributed.get_world_size())), dtype=torch.int32)
        num_groups = cp_size // local_size
        group_ranks = group_ranks.reshape(-1, num_groups, local_size)
        inter_ranks = group_ranks.transpose(1, 2).flatten(0, 1)
        intra_ranks = group_ranks.flatten(0, 1)
        intra_group, _ = torch.distributed.new_subgroups_by_enumeration(
            intra_ranks.numpy().tolist(), backend="nccl"
        )
        inter_group, _ = torch.distributed.new_subgroups_by_enumeration(
            inter_ranks.numpy().tolist(), backend="nccl"
        )
        if args.use_burst and args.use_ulysses:
            print("dq comm is not used in hybird mode")
            args.create_dq_comm = False
        if args.create_dq_comm:
        ...
        if not args.create_dq_comm:
            args.cp_groups_double = (intra_group, inter_group)
        else:
            args.cp_groups_double = ((intra_group, intra_group2), (inter_group, inter_group2)) 
    else:
        args.cp_groups_double = [None, None]
        # cp_g_intra = torch.distributed.new_subgroups(local_world_size, group=mpu.get_context_parallel_group())
    ...
    if not args.skip_train:
        ...
        def _all_print(string):
            torch.distributed.barrier()
            current_time = datetime.now() + timedelta(hours=8)
            time_str = current_time.strftime('%Y-%m-%d %H:%M:%S')
            if torch.distributed.get_rank() == 0:
                assert args.log_file is not None
                mode = "a" if os.path.exists(args.log_file) else "w"
                file = open(args.log_file, mode)
                print('[' + string + '] datetime: {} '.format(time_str), file=file) 
                file.close()
        ...
        def cal_mean_std(data):
            ...
            return mean, std
        flops_mean, flops_std = cal_mean_std(args._flops[3:])
        tgs_mean, tgs_std = cal_mean_std(args._tokens[3:])
        _all_print('*******final summary********')

        _all_print("CP_SIZE {} Method {} Sequence Lenth {}".format(args.context_parallel_size, args.method, args.seq_length))
        _all_print('MFS\t{:.2f}\t±{:.2f}'.format(flops_mean, flops_std))
        _all_print('TGS\t{:.2f}\t±{:.2f}'.format(tgs_mean, tgs_std))
        _all_print('*******summary end********')
...
def training_log(loss_dict, total_loss_dict, learning_rate, decoupled_learning_rate, iteration,
    ...
    if iteration % args.log_interval == 0:
        ...
        if not hasattr(args, '_flops'):
            args._flops = []
            args._tokens = []
        if args.log_throughput:
            tokens_throughput = args.global_batch_size * args.seq_length / elapsed_time / args.world_size
            log_string += f' throughput per GPU (TFLOP/s/GPU): {throughput:.1f} |'
            log_string += f' Token per GPU (Tokens/s/GPU): {tokens_throughput :.1f} |'
            if args.log_timers_to_tensorboard:
                if writer:
                    writer.add_scalar('throughput', throughput, iteration)
                if wandb_writer:
                    wandb_writer.log({'throughput': throughput}, iteration)
            args._flops +=[throughput]
            args._tokens +=[tokens_throughput]
