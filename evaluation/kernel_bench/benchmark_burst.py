...
def init_setting():
    ...
    # setting["seqlen"] = [1024 * 4 * get_world_size(), 1024 * 8 * get_world_size(), 1024 * 16 * get_world_size()]
    setting["seqlen"] = [4, 8, 16, 32]
    setting["seqlen"] = [1024*i*get_world_size() for i in setting['seqlen']]
    # setting["seqlen"] = [ 1024 * 32 * get_world_size()]
    # setting["seqlen"] = [1024 * 4 * get_world_size()]
    setting["num_heads"] = [40]
    setting["dim"] = [128]
    setting["causal"] = [True]
    setting["double_ring"] = [True]
    setting["bwd_opt"] = [True]
...
def benchmark_forward_backward(
    ...
    **kwinputs,
    ...
    if verbose:
        print(desc, "- Forward Pass Backward pass")
    ...
    t = benchmark.Timer(
        stmt="f(*inputs, grad=grad)",
        globals={"f": f, "inputs": inputs,  "grad": grad},
        num_threads=torch.get_num_threads(),
    )
    m = t.timeit(repeats)
    return t, m
...
def benchmark_one_setting(method, settings):
    ...
    if method in ["flash", "burst", "burst_striped"]:
        ...
        _, forward_backward_time = benchmark_forward_backward(
            forward_func,
            *inp,
            causal=causal,
            desc=method,
    ...
    forward_backward_time = forward_backward_time.mean
    ...
    with profiler:
        for i in range(13):
            out = forward_func(*inp, causal=causal, **kwargs)
            out.backward(grad)
            profiler.step()
    ...
    print_rank(
        f"{method}\n\t{head}\n\tTime: | forward: {forward_time*1000:.2f} s | forward_backward: {forward_backward_time*1000:.2f} ms"
    )
    print_rank(
        f"\tTFLOPS| | forward: {fwd_tflops:.2f} TFLOPS | forward_backward: {fwd_bwd_tflops:.2f} TFLOPS | backward: {bwd_tflops:.2f} TFLOPS"
    )
    return forward_time, forward_backward_time, fwd_tflops, fwd_bwd_tflops
...
def run_bench_torch():
    ...
    for i, setting in enumerate(get_setting()):
        ...
        for method in [
            ...
            f, fb, ft, fbt  = benchmark_one_setting(method, setting)
            bench_res.append((setting, f, fb, fbt))
    ...
    if get_rank() == 0:
        ...
        header = f" {'B':^4} | {'Seq Len':^7} |  {'FWD (ms)':^10} | {'FWD+BWD (ms)':^12} | {'TFLOPs/s':^10}"
        print(header)
        print(f"{'-' * 120}")

        for res in bench_res:
            setting, f, fb, fbt = res
            b, seqlen, n, d, causal, double_ring, opt_bwd = setting
            row = (
                f"{str(b):^4} | "
                f"{str(seqlen):^7} | "
                f"{f:^10.2f} | "
...
def run_bench_bmt():
    ...
    for i, settings in enumerate(get_setting()):
        ...
        for method in ["burst", "burst_striped"]:
            if method == "burst_striped" and not causal:
                continue
            f, fb, ft, fbt = benchmark_one_setting(method, settings)
