    ...
    def _update_states(self, fast_skip: bool = True):
        ...
        if total_blocks > 0:
            ...
            nw_unused_block_offset = [
                offset_dict[block] if block in offset_dict else 0
                    for block in nw_unused_block 
            ]
    ...
    def state_dict(self):
        ...
        with torch.no_grad():
            if self._world_size > 1:
                ...
                max_unused_blocks = bmt.distributed.all_reduce(gpu_num_unused_block, op="max", comm=bmt.config["tp_zero_comm"]).cpu().item()
                ...
                global_states = bmt.distributed.all_gather(gpu_states, comm=bmt.config["tp_zero_comm"]).cpu()  # (world_size, max_unused_blocks)
                global_offset = bmt.distributed.all_gather(gpu_offset, comm=bmt.config["tp_zero_comm"]).cpu()  # (world_size, max_unused_blocks)
                global_block = bmt.distributed.all_gather(gpu_block, comm=bmt.config["tp_zero_comm"]).cpu()  # (world_size, 4)
                return {
    ...
    def read(self):
        ...
        if self._curr_block is None:
            ...
            try:
                self._open_file(
                    f_info.file_name,
                    (next_block_id - f_info.block_begin) * f_info.block_size + next_block_offset
                )
