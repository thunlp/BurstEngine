...
class FFNBlock(torch.nn.Module):
    ...
    def forward(
        ...
        if self.dropout is not None:
            x = self.dropout(x)
        hidden_states = hidden_states + x.view_as(hidden_states)  # / 1.05
        return hidden_states
...
class TransformerBlock(torch.nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.ffn_first = "ffn_first" in get_args().sp 
        self.mask_att = mask_att
        self.mask_ffn = mask_ffn
        if self.ffn_first and layer_idx == 0:
            self.mask_ffn = True
        if self.ffn_first and layer_idx == get_args()._config.num_layers:
            self.mask_att = True
    ...
    def forward(
        ...
        if self.ffn_first:
            if not self.mask_ffn:
                self_hidden_states = self.ffn(self_hidden_states)
        if not self.mask_att:
        ...
        # (batch, dim_model, seq_self)
        if not self.mask_ffn and not self.ffn_first:
