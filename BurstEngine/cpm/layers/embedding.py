...
class Embedding(bmt.DistributedModule):
    ...
    def forward(self, ids: torch.Tensor):
        ...
        """  # noqa: E501
        if self.tp and self.tp_sp:
            ids = ids.view(-1).chunk(bmt.config['tp_size'])[bmt.config['tp_rank']].view(ids.size(0), -1)
        elif self.sp:
            ids = ids.chunk(bmt.config['tp_size'],dim=1)[bmt.config['tp_rank']]
        if self.scale:
