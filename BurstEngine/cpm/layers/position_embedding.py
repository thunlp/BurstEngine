...
class RotaryEmbeddingESM(bmt.DistributedModule):
    ...
    def forward(
        self, q: torch.Tensor, k: torch.Tensor, seq_dim, offset=0, max_length=None,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        seq_dim = (seq_dim + k.dim()) % k.dim()
        self._cos_cached, self._sin_cached = self._update_cos_sin_tables(k, seq_dim, k.size(seq_dim) if max_length is None else max_length)
        return (
