...
from ..tokenizers import CaterpillarTokenizer
...
CaterpillarInputType = Union[str, Dict[str, "CaterpillarInputType"]]
...
_TransformFunction = Callable[[CaterpillarInputType, int, random.Random], CaterpillarInputType]


class CaterpillarBatch(TypedDict):
...
def convert_data_to_id(tokenizer: CaterpillarTokenizer, data: Any):
...
class _MixedDatasetBatchPacker:
    def __init__(
        ...
        batch_size: int,
        max_length: int,
        tokenizer: CaterpillarTokenizer,
        unpad: bool = False,
    ) -> None:
    ...
    def apply_transform(
        self,
        data: CaterpillarInputType,
        transform: Union[Dict[str, Any], Callable[[CaterpillarInputType], CaterpillarInputType], None],
    ) -> CaterpillarInputType:
        ...
        def _expand_mapping(data: CaterpillarInputType, stars: List[str], path: List[str], target: List[str]):
            if len(path) == 0:
                num_stars = 0
                for it in target:
                    if it == "*":
    ...
    def _ensure_transform_function(self, module_name: str, transform_script_path: str) -> _TransformFunction:
        ...
        if transform_func is None:
            ...
            def _empty_transform_func(data: CaterpillarInputType, num_sample: int, r: random.Random):
    ...
    def build_instance(self, config: _MixedDatasetConfig):
        ...
        if isinstance(transforms, str):
            ...
            def _transform(data: CaterpillarInputType):
        ...
        while True:
            ...
            if sample is None:
                continue
            sample_input_ids, sample_context = self.data_to_id(sample)

            if input_ids.shape[0] + sample_input_ids.shape[0] > self._clip_length:
                break  # too long, break
            raw_data["samples"].insert(0, sample)
            # use "\n\n" as seperator, do compute loss
            sep = np.array(self.tokenizer.encode("\n\n"), dtype=np.int32)
            input_ids = np.concatenate([sample_input_ids[:-1], sep, input_ids[1:]], axis=0)
            context = np.concatenate([sample_context[:-1], np.array([0, 0], dtype=np.int8), context[1:]], axis=0)
    ...
    def pack_batch(self, force: bool = False, unilm=False) -> CaterpillarBatch:
        # pack batch
    ...
    def add_data(self, config: _MixedDatasetConfig) -> Optional[CaterpillarBatch]:
        (
            input_ids,
            context,
            raw_data,
...
class MixedDataset:
    def __init__(
        ...
        if bmt.config["pipe_size"] > 1:
            maxsize = bmt.config["micros"]
        else:
            maxsize = 2
        self._q_data = multiprocessing.Queue(maxsize=maxsize)
        ...
        self._p = multiprocessing.Process(
            ...
            args=(
                ...
                self._q_cmd_out,
                self._q_data,
                bmt.rank() if rank is None else rank,
                bmt.world_size() if world_size is None else world_size,
                self._packer,
                max_repeat_times,
                random.getstate(),
            ),
    ...
    def get(self) -> CaterpillarBatch:
        ret: CaterpillarBatch = self._q_data.get(timeout=300)  # type: ignore
