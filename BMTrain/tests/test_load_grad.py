...
class TestSubModule(bmt.DistributedModule):
    def __init__(self):
        ...
        self.fc1 = bmt.BMTrainModelWrapper(torch.nn.Linear(768, 3072, dtype=dtype))
        self.fc2 = bmt.BMTrainModelWrapper(torch.nn.Linear(3072, 1024, dtype=dtype))
        self.fc3 = bmt.BMTrainModelWrapper(torch.nn.Linear(1024, 768, dtype=dtype))
        self.param = bmt.DistributedParameter(torch.zeros(1237, dtype=dtype))
        self.fc4 = bmt.BMTrainModelWrapper(torch.nn.Linear(768, 300, dtype=dtype))
        self.fc5 = bmt.BMTrainModelWrapper(torch.nn.Linear(300, 768, dtype=dtype))
